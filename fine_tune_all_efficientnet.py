...
from efficient_v2_classifier import EfficientNetV2_Classifier
...
IMG_SIZE = 280         # final size for model input
BATCH_SIZE = 64
EPOCHS_HEAD = 5
EPOCHS_FINE = 10
...
for model_name, BaseModelClass in all_base_models.items():
    ...
    if TRAIN_HEAD:
        efficientnet_classifier = EfficientNetV2_Classifier(
    ...
    if TRAIN_FINE:
        if not TRAIN_HEAD:
            ...
            efficientnet_classifier.model.load_weights(
                "output/efficientnet_head_{model_name}.weights.h5"
            )
        ...
        # SAVE MODEL
        # =====================
        efficientnet_classifier.model.save_weights("output/efficientnet_fine_{model_name}.weights.h5")
        print("\n Model saved!")
